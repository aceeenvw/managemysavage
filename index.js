...
function showDirectionModal(key, fetishName) {
    ...
    return new Promise((resolve) => {
        ...
        $('#fm-modal-giving-desc').text(config.giving);
        $('#fm-modal-receiving-desc').text(config.receiving);
    ...
    state.active.forEach(k => {
        ...
        if (match) {
            ...
            if (baseFetish && dirConfig) {
                // Modify the prompt to specify direction
                let modifiedPrompt = baseFetish.prompt;
                
                if (direction === 'giving') {
                    // Replace verbs to indicate active role
                    modifiedPrompt = modifiedPrompt.replace(
                        /(enjoys|loves|is aroused by) ([^,]+or [^,]+)/,
                        `$1 ${dirConfig.giving}`
                    );
                } else {
                    // Replace verbs to indicate passive role
                    modifiedPrompt = modifiedPrompt.replace(
                        /(enjoys|loves|is aroused by) ([^,]+or [^,]+)/,
...
function toggle(key) {
    ...
    // Check if this is a directional fetish being ADDED
